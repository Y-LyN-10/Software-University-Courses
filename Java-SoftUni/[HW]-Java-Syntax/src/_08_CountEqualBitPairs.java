...
public class _08_CountEqualBitPairs {
	...
	public static void main(String[] args) {
		...
		try (Scanner input = new Scanner(System.in)) {
			...
			// in Java binary numbers always have 32 bits (no matter if zeroes
			// or not) so we find first the number of leading zeroes then we 
			//find the index of the last 1 bit
			...
			for (int i = 0; i < indexLastBit; i++) {
				...
				Boolean cond01 = (n & mask01) == mask01; //checks if the current bit is 0 or 1
				Boolean cond02 = (n & mask02) == mask02; //checks if the next bit is 0 or 1
