...
public class _10_PrintHouse {
	...
	public static void main(String[] args) {
		...
		try (Scanner input = new Scanner(System.in)) {
			...
			double leftSide = ((12.5 - 17.5) * (data[1] - 3.5) - (8.5 - 3.5)
					* (data[0] - 17.5));
			double rightSide = ((22.5 - 17.5) * (data[1] - 3.5) - (8.5 - 3.5)
					* (data[0] - 17.5));
			...
			boolean leftWall = data[1] >= 8.5 && data[1] <= 13.5
					&& data[0] >= 12.5 && data[0] <= 17.5;
			boolean rightWall = data[1] >= 8.5 && data[1] <= 13.5
					&& data[0] >= 20 && data[0] <= 22.5;
			...
			// then we add the circle (with different color, depending if inside
			// or outside)
			if (roof == true || walls == true) {
				// inside
				content += "<circle cx=\"" + pixelsX + "\" cy=\"" + pixelsY
						+ "\" r=\"5\" stroke=\"black\" fill=\"pink\" />\n";
			} else {
				// outside
				content += "<circle cx=\""
						+ pixelsX
						+ "\" cy=\""
						+ pixelsY
						+ "\" r=\"5\" stroke=\"black\" stroke-width=\"1\" fill=\"grey\" />\n";
			}
